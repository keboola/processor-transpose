...
class Transpose
{
    ...
    protected function getInputFile($filename)
    {
        ...
        return new CsvFile($inputFilePath);
    }
    ...
    protected function getTransposeFn($config, CsvFile $output, $csvHeaderRaw, $csvTransposeHeader) {
        $transposeFrom = $config['transpose_from_column'];
        $transposedHeaderColumnName = $config['header_transpose_column_name'];

        return function ($csvRow) use (
            $transposeFrom,
            $output,
            $transposedHeaderColumnName,
            $csvHeaderRaw,
            $csvTransposeHeader
        ) {
            ...
            foreach ($transposeCsvRow as $k => $v) {
                ...
                if (!is_null($csvTransposeHeader) && !empty($csvTransposeHeader[$k])) {
                    $outRowArr[$transposedHeaderColumnName] = $csvTransposeHeader[$k];
                }

                $output->writeRow($outRowArr);
            }
        };
    }

    protected function transposeHeader($csvHeader, $config)
    {
        $csvOutHeaderArr = array_slice($csvHeader, 0, ($config['transpose_from_column'] - 1));
        $csvOutHeaderArr[] = 'key';
